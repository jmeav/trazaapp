...
class SendRepoView extends StatelessWidget {
  ...
  void _showErrorDialog(BuildContext context, String title, String errorDetails) {
    Get.dialog(
      AlertDialog(
        title: Text(title),
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              const Text('Detalles del error:', style: TextStyle(fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Text(errorDetails),
              const SizedBox(height: 16),
              const Text('Últimos logs:', style: TextStyle(fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              ...debugLogs.map((log) => Text(log, style: const TextStyle(fontSize: 12))),
            ],
          ),
        ),
        actions: [
                                ...
                                Row(
                                  ...
                                  children: [
                                    ...
                                    _buildActionButton(
                                      ...
                                      onPressed: () async {
                                        try {
                                          await controller.enviarReposicion(repo.idRepo);
                                          // Get.snackbar(
                                          //   'Éxito',
                                          ...
                                          print("❌ Error capturado en la vista: ${e.toString()}");
