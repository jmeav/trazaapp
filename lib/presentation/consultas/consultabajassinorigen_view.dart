...
class ConsultaBajaSinOrigenView extends StatelessWidget {
  const ConsultaBajaSinOrigenView({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final ConsultasBajasSinOrigenController controller = Get.find();
    ...
    void mostrarEvidencia(String evidenciaUrl) async {
      if (evidenciaUrl.isNotEmpty) {
        final Uri url = Uri.parse(evidenciaUrl);
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Consulta de Bajas Sin Origen'),
        actions: [
          GetBuilder<ConsultasBajasSinOrigenController>(
                ...
                Expanded(
                  child: Obx(() {
                    ...
                    if (controller.filteredResultados.isEmpty && !controller.isLoading.value) {
                      return const Center(child: Text('No hay bajas sin origen para mostrar con los filtros seleccionados.'));
                    }
                    
                    return controller.isCardView.value
                        ? _buildCardView(controller, getEstadoInfo, context, _dateFormat, calcularDiasPendientes, mostrarEvidencia)
                        : _buildTableView(context, controller, getEstadoInfo, isLandscape, _dateFormat, calcularDiasPendientes, mostrarEvidencia);
                  }),
                ),
              ],
            );
  ...
  Widget _buildFilterControls(BuildContext context, ConsultasBajasSinOrigenController controller, String token, String codhabilitado) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
        ...
        Obx(() => controller.isLoading.value
            ? const Center(child: Column(
                children: [
                  ...
                  SizedBox(height: 8),
                  Text('Consultando bajas sin origen, por favor espere...'),
                ],
              ))
            : ElevatedButton(
                onPressed: () {
                  controller.consultarBajasSinOrigen(
                    token: token,
  ...
  Widget _buildCardView(
    ConsultasBajasSinOrigenController controller, 
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final String arete = item['arete']?.toString() ?? 'N/A';
        final String fechaRegistroStr = item['fecha_registro']?.toString() ?? '';
        final String fechaStr = item['fecha']?.toString() ?? '';
        final String evidenciaUrl = item['evidenciaUrl']?.toString() ?? '';
        ...
        DateTime? fechaDate;
        try {
          if (fechaStr.isNotEmpty) fechaDate = DateTime.parse(fechaStr);
        } catch (e) {}
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text('Arete: $arete', style: const TextStyle(fontSize: 14)),
                        if (fechaRegistroDate != null)
                          Text('Fecha Registro: ${dateFormat.format(fechaRegistroDate)}', style: const TextStyle(fontSize: 14)),
                        if (fechaDate != null)
                          Text('Fecha: ${dateFormat.format(fechaDate)}', style: const TextStyle(fontSize: 14)),
                        ...
                        if (motivoRechazoOriginal.isNotEmpty && motivoRechazoOriginal != "0000-00-00 00:00:00")
                          Padding(
                            ...
                            child: Text('Motivo rechazo: $motivoRechazoOriginal', style: const TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
                          ),
                      ],
                    ),
                  ),
          ...
          child: Column(
            children: [
              Container(
                ...
                child: Row(
                  children: [
                    _buildTableHeader('ID', width: 100),
                    _buildTableHeader('Arete', width: 130),
              ...
              ...controller.filteredResultados.map((item) {
                final estado = item['estadoproceso']?.toString() ?? '0';
                final estadoInfo = getEstadoInfo(estado);
                final String id = item['idBaja']?.toString() ?? 'N/A';
                final String arete = item['detallearetes']?[0]?['arete']?.toString() ?? 'N/A';
                final String fechaRegistroStr = item['fecharegistro']?.toString() ?? '';
                final String fechaStr = item['fechabaja']?.toString() ?? '';
                final String motivoRechazoOriginal = item['motivorechazo']?.toString() ?? '';
                DateTime? fechaRegistroDate;
                try {
                  if (fechaRegistroStr.isNotEmpty) fechaRegistroDate = DateTime.parse(fechaRegistroStr);
                } catch (e) {}
                ...
                return Container(
                  ...
                  child: IntrinsicHeight(
                    child: Row(
                      ...
                      children: [
                        _buildTableCell(id, width: 100),
                        _buildTableCell(arete, width: 130),
                        _buildTableCell(fechaRegistroDate != null ? dateFormat.format(fechaRegistroDate) : 'N/A', width: 150),
                        _buildTableCell(fechaDate != null ? dateFormat.format(fechaDate) : 'N/A', width: 150),
                        Container(
                          width: 100,
                          padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 4),
